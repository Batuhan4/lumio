...
pub fn compute_charge(rates: &UsageMeterRates, usage: &UsageBreakdown) -> Option<i128> {
    ...
    total = total
        .checked_add(rates.llm_in.checked_mul(usage.llm_in)?)
        ?;
    total = total
        .checked_add(rates.llm_out.checked_mul(usage.llm_out)?)
        ?;
    total = total
        .checked_add(rates.http_calls.checked_mul(usage.http_calls)?)
        ?;
    total = total
        .checked_add(rates.runtime_ms.checked_mul(usage.runtime_ms)?)
        ?;
    Some(total)
}
